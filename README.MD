...
在`0.5.0`之后的版本，我们支持了更加可定制化的人格设置。新的人格设置从txt加载文件，并且有一定的语法规则：

1. 人格文件的格式为txt，每条设置之间都需要空出来一行。
2. 人格文件的配置以 `key: value` 的形式进行配置，其中key为配置的名称，value为配置的值，value可以占用多行，但是需要遵循上面的设置规则。
3. 人格文件的注释以`#`开头，`#`后面一行的内容都会被忽略。

你可以点这里来查看我们默认附带的人格文件：[catgirl.txt](/packages/core/dist/presets/catgirl.txt)

我们的人格文件夹路径为 你当前运行插件的koishi目录的路径+`/chathub/presets`，你可以在插件配置中修改这个路径。

所有的人格文件都是从上面的文件夹上加载的。因此你可以自由添加和编辑人格文件在这个文件夹下，然后使用命令来切换人格。
