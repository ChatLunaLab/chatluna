...
export class PromptGenerator {
    ...
    _generate_numbered_list(items: any[], item_type = "list"): string {
        if (item_type === "command") {
            ...
            const finish_string = `${items.length + 1
...
export function generateSearchAndChatPrompt(tools: ObjectTool[]): string {
    const prompt_generator = new PromptGenerator();

    prompt_generator.add_constraint(
        "You need to decide what tools you use to get information based on what the user is currently talking about, And use the tool only when users talk to you about things you don't understand! If you don't need the tool, call the finish tool directly to complete the task based on the content of our conversation.")

    prompt_generator.add_constraint(
        "~2500 word limit for short term memory. " +
