import { ChatMessage, createSystemChatMessage } from '../common/scheme.js';
import { BaseLanguageModel } from '../model/base.js';
...
export interface ConversationSummaryMemoryInput extends BaseChatMemoryInput {
    ...
    summaryChatMessageClass?: (content: string) => ChatMessage;
}
...
export class ConversationSummaryMemory extends BaseChatMemory {
    ...
    summaryChatMessageClass = (content: string): ChatMessage =>
        createSystemChatMessage(content)
    ...
    async loadMemoryVariables(_: InputValues): Promise<MemoryVariables> {
        if (this.returnMessages) {
            const result = {
                [this.memoryKey]: [this.summaryChatMessageClass(this.buffer)],
            };
            return result;
        }
