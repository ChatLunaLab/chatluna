...
export function apply(ctx: Context, config: Config, chain: ChatChain) {
    ...
    ctx.command('chathub.room.delete [room:text]', '删除一个房间').action(async ({ session }, room) => {
    ...
    ctx.command('chathub.room.kick <...arg:user>', '踢出某个人员在你当前的房间').action(async ({ session }, ...user) => {
    ...
    ctx.command('chathub.room.invite <...arg:user>', '邀请进入房间').action(async ({ session }, ...user) => {
        const users = user.map((u) => u.split(':')[1])
    ...
    ctx.command('chathub.room.leave [room:text]', '离开当前房间').action(async ({ session, options }, room) => {
    ...
    ctx.command('chathub.room.clear [room:text]', '清除房间的聊天记录').action(async ({ session }, room) => {
    ...
    ctx.command('chathub.room.transfer <arg:user>', '转移房间的房主').action(async ({ session }, user) => {
    ...
    ctx.command('chathub.room.info [room:text]', '查看当前房间的信息').action(async ({ session }, room) => {
    ...
    ctx.command('chathub.room.switch <name:text>', '切换到你已经加入了的房间').action(async ({ session }, name) => {
    ...
    ctx.command('chathub.room.permission <user:user>', '修改房间里某人的权限').action(async ({ session }, user) => {
