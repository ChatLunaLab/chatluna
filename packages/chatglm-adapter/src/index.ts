...
export function apply(ctx: Context, config: Config) {
    ...
    ctx.on("ready", async () => {
        ...
        await plugin.registerClient((_, clientConfig) => new OpenLLMClient(ctx, config, clientConfig))
...
export const Config: Schema<Config> = Schema.intersect([
    ...
    Schema.object({
        apiKeys: Schema.array(
            Schema.tuple([
                ...
                Schema.string().description('请求 API for Open LLMs 自搭建后端的地址').default("http://127.0.0.1:8000")
            ]))
    ...
    Schema.object({
        maxTokens: Schema.number().description('回复的最大 Token 数（16~8000，必须是16的倍数）（注意如果你目前使用的模型的最大 Token 为 8000 及以上的话才建议设置超过 512 token）')
            .min(16).max(8000).step(16).default(1024),
